...
$$
u_s\, b_i \;\ge\; b_i - R^{\mathrm{in}}_{is} - b_i\,\zeta \tag{SLA-RU-in}
$$

$$
u_s\, b_i \;\ge\; b_i - R^{\mathrm{out}}_{is} - b_i\,\zeta \tag{SLA-RU-out}
$$

$$
u_s \ge 0 \tag{SLA-RU-nonneg}
$$
...
$$
\phi_s \;\ge\; \frac{D_{m,k} - C^{\mathrm{N}}_{m,k,s}}{\bar D_k} - \zeta^{\mathrm{sf}} \tag{SF-RU}
$$

$$
\mathrm{CVaR}^{\mathrm{SF}}_\beta(L^{\mathrm{SF}}) = \zeta^{\mathrm{sf}} + \frac{1}{1-\beta_{\mathrm{sf}}}\sum_{s\in\mathcal{S}} \pi_s\, \phi_s \tag{SF-CVaR}
$$

$$
D_{m,k} = \sum_i w_{i,k}\, y_{i,m} \tag{SF-demand}
$$
...
$$
\sum_{m\in\mathcal{M}} y_{i,m} = 1 \qquad \forall i\in\mathcal{I} \tag{assign}
$$
...
$$
\sum_{p} x^{\mathrm{in}}_{i,m,p} \;= y_{i,m}\, b_i \qquad \forall i,m \tag{flow-in-ub}
$$

$$
\sum_{q} x^{\mathrm{out}}_{i,m,q} \;= y_{i,m}\, b_i \qquad \forall i,m \tag{flow-out-ub}
$$
...
$$
\sum_{i\in\mathcal{I}} w_{i,k}\, y_{i,m} \;\le\; C^{\mathrm{norm}}_{m,k} \qquad \forall m,k \tag{cap-normal}
$$
...
$$
\sum_{i,m,p} x_{i,m,p}^{in}\,\delta_{e,p} + \sum_{i,m,q} x_{i,m,q}^{out}\,\delta_{e,q} \;\le\; B_e \qquad \forall e\in\mathcal{E} \tag{link-cap}
$$
...
$$
d^{\mathrm{in}}_{i,m,p,s} = x^{\mathrm{in}}_{i,m,p} \quad \text{若路径 } (u_i,m,p) \text{ 在 } s \text{ 可用} \tag{del-in-eq}
$$

$$
d^{\mathrm{in}}_{i,m,p,s} = 0 \quad \text{若该路径在 } s \text{ 不可用} \tag{del-in-zero}
$$

$$
d^{\mathrm{out}}_{i,m,q,s} = x^{\mathrm{out}}_{i,m,q} \quad \text{若路径 } (m,v_i,q) \text{ 在 } s \text{ 可用} \tag{del-out-eq}
$$

$$
d^{\mathrm{out}}_{i,m,q,s} = 0 \quad \text{若该路径在 } s \text{ 不可用} \tag{del-out-zero}
$$
...
$$
R^{\mathrm{in}}_{is} \;\le\; \sum_{m} b_i\, y_{i,m}\, \sigma^{\mathrm{vs}}_{m,s} \tag{vs-cap-in}
$$

$$
R^{\mathrm{out}}_{is} \;\le\; \sum_{m} b_i\, y_{i,m}\, \sigma^{\mathrm{vt}}_{m,s} \tag{vs-cap-out}
$$
...
$$
\mathrm{CVaR}^{\mathrm{SLA}}_\beta(L^{\mathrm{SLA}}) \;\le\; \Gamma_{\mathrm{sla}} \tag{Gamma-SLA}
$$

$$
\mathrm{CVaR}^{\mathrm{SF}}_\beta(L^{\mathrm{SF}}) \;\le\; \Gamma_{\mathrm{sf}} \tag{Gamma-SF}
$$
