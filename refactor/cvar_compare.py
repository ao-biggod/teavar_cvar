...
def add_compute_sf_cvar_ru(
    ...
    *,
    ...
    del_in: dict | None = None,
    xin: dict | None = None,
):
    """
    算力 SF CVaR：Rockafellar–Uryasev 纯连续形式。

    **v2（端到端修复）**：shortfall 基于「到达数据量 vs 节点处理能力」，
    而非仅看放置需求 sum(y*w)。当链路故障导致数据无法到达节点时，
    节点实际负载降低，shortfall 相应减少——自然产生网络↔算力交叉效应。

    actual_load[m, k, s] = sum_i( arrived_i[m, s] * w[i,k] / b_in[i] )
      其中 arrived_i[m, s] = sum_p( del_in[i, m, p, s] )  （场景 s 下到达 m 的数据量）

    phi_s[s] >= (actual_load[m,k,s] - C_s[m,k,s]) / D_ref[k] - zeta_sf
    """
    refs = sf_d_ref_by_resource if sf_d_ref_by_resource is not None else compute_sf_resource_refs(data)
    has_flow = del_in is not None and xin is not None

    actual_load: dict[tuple[int, int, int], gp.Var] = {}
    for node in data.M:
        for k in data.K:
            for s in data.S:
                if has_flow:
                    # 端到端：基于场景下实际到达该节点的数据量计算负载
                    arrived_terms = []
                    for i in data.I:
                        if (i, node) not in y:
                            continue
                        ...
                        arr_i = gp.quicksum(
                            del_in[i, node, p, s]
                            for p in range(len(data.P_cand.get(
                                (data.task_src.get(i, data.hub), node), []
                            )))
                            if (i, node, p, s) in del_in
                        )
                        # 按资源维度加权：负载比例 = w[i,k] / b_in[i]
                        ratio = data.w[i][k] / max(data.b_in[i], 1e-12)
                        arrived_terms.append(arr_i * ratio)
                    actual_load[node, k, s] = m.addVar(lb=0.0, name=f"aload_{node}_{k}_{s}")
                    m.addConstr(
                        actual_load[node, k, s] == gp.quicksum(arrived_terms) if arrived_terms else 0.0,
                        name=f"aload_def_{node}_{k}_{s}",
                    )
                else:
                    # 降级：无流量变量时回退到放置需求（兼容旧调用）
                    d_mk_var = m.addVar(lb=0.0, name=f"dreq_{node}_{k}")
                    m.addConstr(
                        d_mk_var == gp.quicksum(
                            y[i, node] * data.w[i][k]
                            for i in data.I if (i, node) in y
                        ),
                        name=f"ddef_{node}_{k}",
                    )
                    actual_load[node, k, s] = d_mk_var
...
def build_teavar_sla_cvar_model(
    ...
    if beta_loss is None:
        beta_loss = data.alpha_N

    use_node_cvar = bool(lambda_node_cvar and lambda_node_cvar > 0.0)
    b_node = beta_node if beta_node is not None else data.alpha_N

    use_compute_sf = bool(lambda_compute_sf_cvar and lambda_compute_sf_cvar > 0.0)
    b_sf = beta_compute_sf if beta_compute_sf is not None else data.alpha_N
    ...
    actual_load = {}
    if use_compute_sf:
        zeta_sf, phi_s, shortfall_cvar, actual_load = add_compute_sf_cvar_ru(
            m, data, y, beta_sf=b_sf, del_in=del_in, xin=xin,
        )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="B",
        help="teavar_sla 需求损失 CVaR 的 α（默认用数据 alpha_N）。调低可改变尾部 (1-alpha) 质量，有时使 loss 随放置分化从而让 λ 影响整数解。",
    )
    ap.add_argument(
        "--lambda-node",
        type=float,
    ...
    ap.add_argument(
        ...
        default=None,
        metavar="BN",
        help="算力 CVaR 的 α（默认与数据 alpha_N 相同）。",
    )
    ap.add_argument(
        "--lambda-compute-sf",
        type=float,
    ...
    ap.add_argument(
        ...
        metavar="BSF",
        help="算力未满足 CVaR 的 α（默认与数据 alpha_N 相同）。",
    )
    args = ap.parse_args()
    ...
    print(
        "beta_loss(teavar) = "
        + (str(args.beta_loss) if args.beta_loss is not None else f"{data_tev.alpha_N} (数据默认)")
        + "  |  beta_node = "
        + (str(args.beta_node) if args.beta_node is not None else f"{data_tev.alpha_N} (数据默认)")
        + "  |  beta_compute_sf = "
        + (str(args.beta_compute_sf) if args.beta_compute_sf is not None else f"{data_tev.alpha_N} (数据默认)")
    )
