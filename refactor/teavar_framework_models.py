...
def build_teavar_model_c(
    ...
    *,
    ...
    b_sf = beta_sf if beta_sf is not None else data.alpha_N
...
if __name__ == "__main__":
    ...
    if "--omega" in argv:
        try:
            omega = float(argv[argv.index("--omega") + 1])
    ...
    if use_toy:
        ...
        d = UltraComplexData()
        print("数据: UltraComplexData (--toy)\n")
    else:
        d = load_b4_joint_data(base_path=base, topology_name="B4", k_paths=6)
        print(f"数据: B4 | hub={getattr(d, 'hub', 0)} | |I|={len(d.I)} | omega={omega}\n")

    lam_sla, lam_sf = 0.5, 0.5
    print("=== TEAVAR Model A (weighted) ===")
    m1, c1, l1, s1, y1, *_ = build_teavar_model_a(d, lam_sla, lam_sf, omega_deliver=omega)
    print(f"  status={m1.status}  cost={c1}  CVaR_SLA={l1}  CVaR_sf={s1}  部署={_dist_str(d, y1) if c1 else 'n/a'}")

    print("\n=== TEAVAR Model C (epsilon, 示例 Γ) ===")
    g_sla = (l1 or 1.0) * 1.5 if l1 else 1.0
    g_sf = (s1 or 0.1) * 2.0 + 0.01 if s1 else 1.0
    m3, c3, l3, s3, y3, *_ = build_teavar_model_c(d, g_sla, g_sf, omega_deliver=omega, include_sf_budget=True)
    print(f"  status={m3.status}  cost={c3}  CVaR_SLA={l3}  CVaR_sf={s3}  部署={_dist_str(d, y3) if c3 else 'n/a'}")

    print("\n=== TEAVAR Model B (KKT) ===")
    m2, c2, l2, s2, y2, *_ = build_teavar_model_b(d, lam_sla, lam_sf, omega_deliver=omega, kkt_sf=True)
    print(f"  status={m2.status}  cost={c2}  CVaR_SLA={l2}  CVaR_sf={s2}  部署={_dist_str(d, y2) if c2 else 'n/a'}")

    print("\n=== TEAVAR Model D (McCormick) ===")
    m4, c4, l4, s4, y4, *_ = build_teavar_model_d(d, omega_deliver=omega, include_sf=True)
    print(f"  status={m4.status}  cost={c4}  事后CVaR_SLA={l4}  事后CVaR_sf={s4}  部署={_dist_str(d, y4) if c4 else 'n/a'}")
