...
def load_b4_joint_data(
    ...
    :param bandwidth_price_scale: 链路带宽单价尺度 $\\pi_e$ 的全局系数（见 ``duibi_metrics``）。
    :param bandwidth_price_mode: ``uniform`` | ``role_transit`` | ``inverse_capacity`` | ``legacy_inverse_capacity``。
    :param pricing_profile: ``uniform`` | ``copo_v1`` | ``legacy``（见 ``duibi_metrics.configure_pricing_on_data``）。
    :param apply_topology_pricing: 为 True 时启用 copo_v1 role_transit 并按 target bw share 标定 scale。
    ...
    obj.node_role = {m: str(csv_rows[m].get("role", "")).strip() for m in M if m in csv_rows}
    from duibi_metrics import PRICING_PROFILE_COPO_V1, configure_pricing_on_data

    configure_pricing_on_data(
        obj,
        pricing_profile=str(pricing_profile),
        bandwidth_price_mode=bandwidth_price_mode,
        bandwidth_price_scale=bandwidth_price_scale,
        apply_topology_pricing=bool(apply_topology_pricing),
        auto_calibrate_copo=(
            str(pricing_profile).lower() == PRICING_PROFILE_COPO_V1
            and bandwidth_price_scale is None
            and not apply_topology_pricing
        ),
    )
